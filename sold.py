...
def create_url(zipcode, filter,page):
    ...
    if filter == "newest":
        url = "https://www.zillow.com/homes/recently_sold/{0}/houses/{2}-_beds/{3}-_baths/{1}_p/days_sort".format(zipcode,page,beds,baths)
    print(url)
    return url
...
def write_data_to_csv(data):
    ...
    with open("properties-sold-%s.csv" % (zipcode), 'wb') as csvfile:
        fieldnames = ['id',
                'address',
                'city',
                'state',
